...
public class ParkManager extends AbstractUser {
	...
	public ParkManager(String theFname, String theLname, String theEmail,
			String thePark, UserStatus theRole) {
	...
	/*
	 * Prompts user for details of the job and creates a new job with the given
	 * details
	 */
	public void addJob() {
		...
		/*
		 * if (response < currentDate) { System.out.println(
		 * "That date is invalid. Please enter a different date. /n");
		...
		/*
		 * while (duration != "1" && duration != "2"){ System.out.println(
		 * "\nInvalid job duration. Please enter 1 or 2. Duration:"); duration =
		 * IODriver.input.next(); }
		 */
		...
		// Job newJob = new Job(startDate, startTime, duration, this.parkName,
		// this.toString(), description, lightSlots, medSlots, heavySlots);
		// add to job list
